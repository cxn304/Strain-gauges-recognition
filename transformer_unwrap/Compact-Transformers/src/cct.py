...
class CxnTokenizer(nn.Module):
    ...
    def forward(self, x):
        ...
        output_feature.append(x)
        # 能不能return两个值还不一定,flattener输出[n, 512, 256]
...
class CXNTransformerUnetWithNoOrigin(nn.Module):
    ...
    def __init__(self, features,
        ...
        '''
        self.upsampling = nn.UpsamplingBilinear2d(scale_factor=2)
        self.conv_up_768_64 = Conv2dReLU(512,64,kernel_size=3,padding=1,stride=1,
            use_batchnorm=False,)
        self.conv_up_192_16 = Conv2dReLU(128,16,kernel_size=3,padding=1,stride=1,
            use_batchnorm=False,)
