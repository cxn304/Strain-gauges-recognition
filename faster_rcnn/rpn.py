...
class RPNplus(tf.keras.Model):
    ...
    def call(self, x, training=False):
        ...
        conv_cls_scores = self.scores_conv(region_proposal) # [1, 64, 80, 18]
        conv_cls_bboxes = self.bboxes_conv(region_proposal) # [1, 64, 80, 36]

        cls_scores = tf.reshape(conv_cls_scores, [-1, 64, 80, 9, 2])
        cls_bboxes = tf.reshape(conv_cls_bboxes, [-1, 64, 80, 9, 4])
